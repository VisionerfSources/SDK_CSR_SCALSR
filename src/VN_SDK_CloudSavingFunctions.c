 ...
 * \retval error code otherwise
 */
VN_tERR_Code VN_SaveAsPcd(VN_Point_XYZI16 *pCloud_XYZI16,
                          int cloudSize,
                          const char *pFileName)
{
    ...
    fprintf(fptr,"WIDTH %d\n",cloudSize);//cloud of points is unorganized -> width is the number of points
    fprintf(fptr,"HEIGHT 1\n");//cloud of points is unorganized -> height is 1
    fprintf(fptr,"VIEWPOINT 0 0 0 1 0 0 0\n");//Default viewpoint
    fprintf(fptr,"POINTS %d\n",cloudSize);//the number of points
 ...
 *
 * \param[in] pMatrix_XYZi   : pointer to a buffer storing the matrix
...
VN_tERR_Code VN_SaveAsDepthMapBmp(VN_Point_XYZI8 *pMatrix_XYZI8, int rowCount, int columnCount, const char *pFileName)
{
    ...
    {
        VN_Point_XYZI8 pt = pMatrix_XYZI8[ptIndex];
 ...
 * \retval error code otherwise
 */
VN_tERR_Code VN_SaveMatrixCloudAsPCD(VN_Point_XYZRGB *pCloud_XYZRGB,
                                     int rowsNumber,
                                     int columnsNumber,
                                     const char *pFileName)
{
...
VN_tERR_Code VN_SaveMatrixXYZRGBAsDepthMapBmp(VN_Point_XYZRGB *pMatrix_XYZRGB, int rowCount, int columnCount, const char *pFileName)
{
    ...
    for(int ptIndex=0 ; ptIndex<(rowCount*columnCount) ; ptIndex++)
    {
        VN_Point_XYZRGB pt = pMatrix_XYZRGB[ptIndex];
        float z=pt.z;

        if (!isnan(z))//i.e. if the point is non-nan, because we can't generate a point if there is no light on it...
        {
            if (initDone == 0)
            {//this is the first point found in the matrix, use it to initialize min & max
                zmin=zmax=z;
